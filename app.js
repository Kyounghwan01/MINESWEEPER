  ...
  for (let i = 0; i < hor; i++) {
    ...
    for (let j = 0; j < hor; j++) {
      ...
      td.addEventListener("click", function(e) {
        ...
        }else{ //지뢰 아닐때 주위의 지뢰 개수 표현
          var 주변 = [
            dataset[blankCol][blank-1], dataset[blankCol][blank+1],
          ];
          if (dataset[blankCol-1]) {
            주변 = 주변.concat([dataset[blankCol-1][blank-1], dataset[blankCol-1][blank], dataset[blankCol-1][blank+1]]);
          }
          if (dataset[blankCol+1]) {
            주변 = 주변.concat([dataset[blankCol+1][blank-1], dataset[blankCol+1][blank], dataset[blankCol+1][blank+1]]);
          }
...
function rightClick(e) {
  ...
  if (
    ...
    mineCounter.textContent = mine -= 1;
